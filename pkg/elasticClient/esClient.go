...
func (this *EsClientHandler) EsClientGetInfoById(id, indexName, typeName string) map[string]interface{} {
	...
	res, err := global2.ES.Get().Index(indexName).Type(typeName).Id(id).Do(context.Background())
	if err != nil {
		return result
	}
	if res == nil {
...
func (this *EsClientHandler) EsClientInsert(id, indexName, typeName string, doc interface{}) error {
	_, err := global2.ES.Index().Index(indexName).Type(typeName).Id(cast.ToString(id)).BodyJson(doc).Refresh("wait_for").Do(context.Background())
...
func (this *EsClientHandler) EsClientUpdateById(id, indexName, typeName string, doc map[string]interface{}) error {
	_, err := global2.ES.Update().Index(indexName).Type(typeName).Id(id).Doc(doc).Refresh("wait_for").Do(context.Background())
	return err
}

//根据id删除数据
func (this *EsClientHandler) DeleteById(id, indexName, typeName string) error {
	_, err := global2.ES.Delete().Index(indexName).Type(typeName).Id(id).Do(context.Background())
	return err
}
